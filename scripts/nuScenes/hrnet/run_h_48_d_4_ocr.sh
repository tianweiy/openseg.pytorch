...
DATA_DIR="${DATA_ROOT}/nuScenes"
SAVE_DIR="${DATA_ROOT}/seg_result/nuScenes/"
BACKBONE="hrnet48"

CONFIGS_TEST="configs/nuScenes/H_48_D_4_TEST_DEPTH.json"

MODEL_NAME="hrnet_w48_ocr"
LOSS_TYPE="fs_auxce_loss"
CHECKPOINTS_NAME="${MODEL_NAME}_"$2
LOG_FILE="./log/nuScenes/${CHECKPOINTS_NAME}.log"
...
if [ "$1"x == "train"x ]; then
  ...
  if [ "$3"x == "ss"x ]; then
    ...
    python main.py --configs ${CONFIGS_TEST} --drop_last no \
                         --backbone ${BACKBONE} --model_name ${MODEL_NAME} --checkpoints_name ${CHECKPOINTS_NAME} \
                         --phase test --gpu 0 1 2 3 --resume ./checkpoints/cityscapes/${CHECKPOINTS_NAME}_latest.pth \
                         --test_dir ${DATA_DIR}/test --log_to_file n \
